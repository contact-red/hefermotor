...
//
// Generated by `tools/tokens/gen_token_kinds.py` from ponyc's
// `src/libponyc/ast/lexer.c` at commit 6a0bfa80b (sha256 of the file:
// 579e239cbf444b7edd0053474250a44b06e0676d46ef1876f93171114c7bbbb8).
// The kinds are ponyc's `symbols` and `keywords` tables verbatim,
// plus the tokens whose text varies, plus the trivia ponyc skips and
// the end and error markers it builds without a table entry. Do not
// edit by hand: `make regen-token-kinds PONYC_SRC=<checkout>`.
...
primitive _Keywords
  """
  The keyword kind that `s` spells, or `None` when it spells none; for
  a lexer that has just scanned an identifier-shaped word.
  """
...
primitive _Symbols
  """
  The symbols a scanner can reach, in ponyc's table order, which puts
  a symbol before its prefixes -- `<<~` before `<<` before `<` -- so
  that taking the first match takes the longest, as ponyc's scan does.
...
primitive _NewlineForm
  """
  A symbol's newline form. Pony distinguishes a `(` that opens a call
  from one that opens a grouped expression by whether a newline
  precedes it; ponyc's `newline_symbols` does this for every symbol
  that has such a form, and this is that function.
  """
